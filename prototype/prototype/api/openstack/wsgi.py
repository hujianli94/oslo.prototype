...
from prototype.api.openstack import api_version_request as api_version
...
SUPPORTED_CONTENT_TYPES = (
    'application/json',
    'application/vnd.openstack.prototype+json',
)

_MEDIA_TYPE_MAP = {
    'application/vnd.openstack.prototype+json': 'json',
...
class Request(webob.Request):
    ...
    def best_match_content_type(self):
        ...
        if 'prototype.best_content_type' not in self.environ:
            ...
            self.environ['prototype.best_content_type'] = (content_type or
                                                         'application/json')

        return self.environ['prototype.best_content_type']
    ...
    def set_api_version_request(self, url):
        ...
        Microversions starts with /v2, so if a client sends a request for
        version 1.0 or 2.0 with the /v2 endpoint, throw an exception.
        Sending a header with any microversion to a /v1 endpoint will
        be ignored.
        Note that a microversion must be set for the legacy endpoint. This
        will appear as 1.0 for /v1.
        """
        if API_VERSION_REQUEST_HEADER in self.headers and 'v1' in url:
...
class Resource(wsgi.Application):
    ...
    def _process_stack(self, request, action, action_args,
        ...
        context = request.environ.get('prototype.context')
    ...
    def __call__(self, req):
        ...
        if req.api_version_request and not _is_legacy_endpoint(req):
            self.wrapped_exc.headers[API_VERSION_REQUEST_HEADER] = (
                    PROTOTYPE_SERVICE + ' ' + req.api_version_request.get_string())
...
def _set_request_id_header(req, headers):
    context = req.environ.get('prototype.context')
    if context:
