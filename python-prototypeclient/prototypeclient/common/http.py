...
USER_AGENT = 'python-prototypeclient'
...
def authenticated_fetcher(hc):
    """A wrapper around the prototype client object to fetch a template."""
...
class HTTPClient(object):
    ...
    def _http_request(self, url, method, **kwargs):
        ...
        if not url.startswith('/v') and not url.startswith('http'):
            url = '/v{0}{1}'.format(self.api_version, url)
        try:
    ...
    def credentials_headers(self):
        creds = {}
        # NOTE(dhu): (shardy) When deferred_auth_method=password, Prototype
        ...
        # TODO(dhu): Make Keystone v3 work in Prototype standalone mode.  Maye
...
class SessionClient(adapter.LegacyJsonAdapter):
    ...
    def __init__(self, *args, **kwargs):
        # 获取API版本参数
        self.api_version = kwargs.pop('api_version', '1')
        # 确保服务类型正确设置
        service_type = kwargs.pop('service_type', 'prototype')
        # 确保接口类型正确设置
        interface = kwargs.pop('interface', kwargs.pop('endpoint_type', 'public'))
...
def _construct_http_client(*args, **kwargs):
    ...
    api_version = kwargs.pop('api_version', '1')  # 获取API版本

    if session:
        service_type = kwargs.pop('service_type', 'prototype')  # 确保默认为prototype
        interface = kwargs.pop('endpoint_type', None) or kwargs.pop('interface', 'public')
