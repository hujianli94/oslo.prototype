...
def print_list(objs, fields, formatters=None, sortby_index=0,
    ...
    table_string = pt.get_string(**kwargs)
    if sys.version_info[0] == 2:
        print(encodeutils.safe_encode(table_string).decode())
    else:
        print(table_string)
...
def print_dict(d, formatters=None):
    ...
    # Python 2/3 compatibility for dictionary iteration
    if sys.version_info[0] == 2:
        items = d.iteritems()
    else:
        items = d.items()
    ...
    for field, value in items:
        if field in formatters:
            pt.add_row([field, formatters[field](value)])
        else:
            pt.add_row([field, value])

    # Python 2/3 compatibility
    table_string = pt.get_string(sortby='Property')
    if sys.version_info[0] == 2:
        print(encodeutils.safe_encode(table_string).decode())
    else:
        print(table_string)
...
def parse_query_url(url):
    ...
    # Python 2/3 compatibility
    if sys.version_info[0] == 2:
        parsed_params = parse.parse_qs(query_params)
    else:
        parsed_params = parse.parse_qs(query_params)
    return base_url, parsed_params
...
def read_url_content(url):
    ...
    if content:
        try:
            ...
            # Python 2/3 compatibility for base64 encoding
            if sys.version_info[0] == 2:
...
def get_response_body(resp):
    body = resp.content
    # Python 2/3 compatibility for headers
    content_type = ''
    if sys.version_info[0] == 2:
