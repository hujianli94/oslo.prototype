...
class PrototypeShell(object):
    ...
    def _append_global_identity_args(self, parser):
        ...
        parser.add_argument(
            ...
            help=_('Explicitly allow prototypeclient to perform '
    ...
    def get_base_parser(self):
        ...
        parser.add_argument('--version',
                            action='version',
                            version=prototypeclient.__version__,
                            help=_("Shows the client version and exits."))

        parser.add_argument('-d', '--debug',
                            default=bool(utils.env('PROTOTYPECLIENT_DEBUG')),
                            action='store_true',
                            help=_('Defaults to %(value)s.') % {
                                'value': 'env[PROTOTYPECLIENT_DEBUG]'
                            })
        ...
        # os-no-client-auth tells prototypeclient to use token, instead of
        # env[OS_AUTH_URL]
        ...
        parser.add_argument('--prototype-url',
                            default=utils.env('PROTOTYPE_URL'),
                            help=_('Defaults to %(value)s.') % {
                                'value': 'env[PROTOTYPE_URL]'
                            })

        parser.add_argument('--prototype_url',
                            help=argparse.SUPPRESS)

        parser.add_argument('--prototype-api-version',
                            default=utils.env('PROTOTYPE_API_VERSION', default='1'),
                            help=_('Defaults to %(value)s or 1.') % {
                                'value': 'env[PROTOTYPE_API_VERSION]'
                            })

        parser.add_argument('--prototype_api_version',
                            help=argparse.SUPPRESS)
        ...
        parser.add_argument('--include-password',
                            default=bool(utils.env('PROTOTYPE_INCLUDE_PASSWORD')),
                            action='store_true',
                            help=_('Send %(arg1)s and %(arg2)s to prototype.') % {
                                'arg1': 'os-username',
                                'arg2': 'os-password'
                            })
        ...
        if osprofiler_profiler:
            parser.add_argument(
                ...
                help=_('HMAC key to use for encrypting context data '
                       ...
                       'configured in osprofiler middleware in prototype, '
                       'it is specified in the paste configuration '
                       '(/etc/prototype/api-paste.ini). Without the key, '
    ...
    def get_subcommand_parser(self, version):
        ...
        submodule = importutils.import_versioned_module('prototypeclient',
                                                        version, 'shell')
    ...
    def _get_keystone_session(self, **kwargs):
        ...
        if verify is None:
            if insecure:
                ...
                # prototypeclient.common.http.get_system_ca_fle()?
    ...
    def main(self, argv):
        ...
        api_version = options.prototype_api_version
        ...
        if args.func == self.do_help:
            ...
            return 0
        # --os-auth-token is not specified, generate a random one
        if args.os_auth_token is None:
            args.os_auth_token = uuid.uuid4().hex
        ...
        if args.os_no_client_auth:
            if not args.prototype_url:
                raise exc.CommandError(_("If you specify --os-no-client-auth "
                                         "you must also specify a Prototype API "
                                         "URL via either --prototype-url or "
                                         "env[PROTOTYPE_URL]"))
        else:
            ...
            if not args.os_auth_url:
                raise exc.CommandError(_("You must provide an auth url via "
                                         ...
                                         "env[OS_AUTH_URL]"))
        api_version = args.prototype_api_version
        kwargs = {
        ...
        if args.os_no_client_auth:
            # Do not use session since no_client_auth means using prototype to
            # to authenticate
            kwargs = {
                ...
                'insecure': args.insecure,
                'timeout': args.api_timeout,
                'endpoint': args.prototype_url,
                'api_version': api_version  # 只传递给HTTP客户端
            }
        else:
            ...
            kwargs = {
                ...
                'password': args.os_password,
                'include_pass': args.include_password,
                'endpoint_override': args.prototype_url,
                'api_version': api_version  # 传递给HTTP客户端
            }

        client = prototype_client.Client(version=api_version, **kwargs)
    ...
    def do_bash_completion(self, args):
        ...
        The prototype.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main(args=None):
    try:
        ...
        PrototypeShell().main(args)
    except KeyboardInterrupt:
        print(_("... terminating prototype client"))
        sys.exit(130)
